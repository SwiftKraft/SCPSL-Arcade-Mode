...
namespace SwiftArcadeMode.Features.Humans.Perks.Content.Caster
{
    public class SummonAltar : SummonSpell
    {
        ...
        {
            public Timer SpawnTimer = new(10f, false);
            ...
            public bool Spawn()
            {
                Vector3 horizontalPos = Random.insideUnitSphere * Range;
                horizontalPos.y = Position.y + MaxHeight / 2f;

                if (Physics.Raycast(horizontalPos + Vector3.up * (MaxHeight / 2f), Vector3.down, out RaycastHit hit, MaxHeight, LayerMask.GetMask("Default"), QueryTriggerInteraction.Ignore))
                {
                    Vector3 spawnPos = hit.point + Vector3.up;
                    Ghoul summon = spawned.Count > 0 && spawned.Count >= Limit ? spawned[0] : new(Owner.DisplayName + "'s Ghoul", "Ghoul".ApplySchematicPrefix(), Owner.Role, new Vector3(0.5f, 0.5f, 0.5f), spawnPos, Quaternion.identity);
